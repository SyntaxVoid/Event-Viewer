...
class Application(tk.Frame):
    ...
    def load_PMT_traces(self):
        ...
        if not self.draw_pmt_traces_var.get():
            ...
            self.disable_widgets(self.bottom_frame_3_bottom.grid_slaves() + [self.piezo_plot_t0_checkbutton_top,
        ...
        if self.current_PMT_event != self.event:
            try:
                self.PMT_data = ReadBinary.ReadBlock(pmt_data_path) # Create some kind of flag to check if the file is
                                                                    # currently loaded into memory so we can
                self.jump_to_t0_trigger_button["state"] = tk.NORMAL
            except:
                pass #TODO: Raise custom exception to be caught by draw_PMT_traces
            try:
                self.align_data = ReadBinary.ReadBlock(align_data_path)
            except:
                pass # TODO: Same ^
